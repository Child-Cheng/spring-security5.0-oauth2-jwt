...
public class CustomAuthExceptionHandler implements AuthenticationEntryPoint, AccessDeniedHandler {
    ...
    public void commence(HttpServletRequest request, HttpServletResponse response, AuthenticationException authException) throws IOException, ServletException {
        ...
        if (cause instanceof InvalidTokenException) {
            ...
            //Token无效
            response.getWriter().write(JSON.toJSONString("token无效"));
        } else {
            log.error("AuthenticationException : NoAuthentication");
            //资源未授权
            response.getWriter().write(JSON.toJSONString("资源未授权"));
        }
    ...
    public void handle(HttpServletRequest request, HttpServletResponse response, AccessDeniedException accessDeniedException) throws IOException, ServletException {
        ...
        response.getWriter().write(JSON.toJSONString("访问资源的用户权限不足"));
    }
}